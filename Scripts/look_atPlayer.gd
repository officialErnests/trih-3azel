...
func _process(delta: float) -> void:
	...
	$CubeMesh.rotation += (Vector3(rotation_x, rotation_y, rotation_z) - $CubeMesh.rotation) * delta
	# print(Vector3(rotation_x, rotation_y, rotation_z) - $CubeMesh.rotation)
	if (Vector3(rotation_x, rotation_y, rotation_z) - $CubeMesh.rotation).length() <= 0.1:
